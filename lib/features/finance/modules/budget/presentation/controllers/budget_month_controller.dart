...
class BudgetMonthController extends StreamState<AsyncState<BudgetScreenData>> {
  ...
  void init() {
    ...
    AsyncState<List<Goal>> goals = _goalController.state;
    ...
    _subscriptions = [
      _monthPlanController.stream.listen((s) { plans = s; if (s is! AsyncLoading) combine(); }),
      _budgetController.stream.listen((s) { budget = s; if (s is! AsyncLoading) combine(); }),
      _transactionController.stream.listen((s) { transaction = s; combine(); }),
      _debtController.stream.listen((s) { debts = s; combine(); }),
      _plannedPaymentController.stream.listen((s) { payments = s; combine(); }),
      _subscriptionController.stream.listen((s) { subs = s; combine(); }),
      _goalController.stream.listen((s) { goals = s; combine(); }),
    ];
  }
