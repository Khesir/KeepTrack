...
class _WalletDetailViewState extends State<WalletDetailView> {
  ...
  double _balanceDelta(Wallet wallet, TransactionType type, double amount) {
  ...
  Color get _walletColor => widget.wallet.colorHex != null
      ? Color(int.parse(widget.wallet.colorHex!.replaceFirst('#', '0xff')))
      : AppColors.accent;
  ...
  Widget build(BuildContext context) {
    ...
    final icon = IconHelper.fromString(widget.wallet.iconCodePoint);
              ...
              return StreamStateBuilder<AsyncState<List<Goal>>>(
                ...
                builder: (context, goalState) {
                  final goals = goalState is AsyncData<List<Goal>>
                      ? goalState.data
                          .where((g) => g.savingsBucketId == widget.wallet.id)
                          .toList()
                      : <Goal>[];
                              ...
                              return TweenAnimationBuilder<double>(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    _EntryRow(transaction: t, isDark: isDark, currentWallet: widget.wallet),
                                  ],
                                ),
                              );
                            },
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Icon(icon, color: walletColor, size: 15),
          ),
          ...
          _ActionBtn(icon: Icons.edit_outlined, label: 'Edit', onTap: onEdit, isDark: isDark),
          const SizedBox(width: 8),
          _ActionBtn(icon: Icons.add_rounded, label: 'Add Entry', onTap: onAddEntry, isDark: isDark, isPrimary: true),
        ],
      ),
    );
  }
...
class _ActionBtn extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final bg = isPrimary
        ? AppColors.accent
        : (isDark ? Colors.white.withValues(alpha: 0.06) : AppColors.border.withValues(alpha: 0.3));
    final fg = isPrimary
        ? Colors.white
...
class _BalanceSummary extends StatelessWidget {
  ...
  String get _balanceLabel =>
      wallet.type == WalletType.creditCard ? 'AMOUNT OWED' : 'CURRENT BALANCE';
  ...
  Widget build(BuildContext context) {
    return TweenAnimationBuilder<double>(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: walletColor.withValues(alpha: isDark ? 0.08 : 0.05),
          borderRadius: BorderRadius.circular(14),
          border: Border.all(color: walletColor.withValues(alpha: 0.2), width: 0.5),
        ),
        child: Row(
          children: [
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  TweenAnimationBuilder<double>(
                    tween: Tween(begin: 0.0, end: wallet.balance),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          if (trailing != null) ...[
            const Spacer(),
            Text(trailing!, style: GoogleFonts.dmSans(fontSize: 11, color: AppColors.textTertiary)),
          ],
        ],
      ),
    );
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    alignment: Alignment.center,
                    child: Icon(IconHelper.fromString(goal.iconCodePoint), color: _goalColor, size: 14),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(goal.name, style: GoogleFonts.dmSans(fontSize: 12, fontWeight: FontWeight.w600, color: fg), maxLines: 1, overflow: TextOverflow.ellipsis),
                        if (budgetName != null)
                          Text(budgetName!, style: GoogleFonts.dmSans(fontSize: 10, color: AppColors.textSecondary), maxLines: 1, overflow: TextOverflow.ellipsis),
                      ],
                    ),
                  ),
                ],
              ...
              Text(
                ...
                style: GoogleFonts.dmMono(fontSize: 10, color: AppColors.textSecondary),
              ),
            ],
          ),
        ),
      ...
      child: Center(
        child: Column(
          children: [
            ...
            Text('No entries yet', style: AppTextStyles.bodySmall.copyWith(color: AppColors.textSecondary)),
            const SizedBox(height: 4),
            Text('Tap Add Entry to record your first transaction', style: AppTextStyles.caption, textAlign: TextAlign.center),
          ],
        ),
      ),
    );
...
class _EntryRow extends StatelessWidget {
  ...
  const _EntryRow({required this.transaction, required this.isDark, required this.currentWallet});

  @override
  Widget build(BuildContext context) {
    final isDeposit = transaction.type == TransactionType.income;
    final isTransfer = transaction.type == TransactionType.transfer;
    final isReceivedTransfer = isTransfer && transaction.toWalletId == currentWallet.id;
    final fg = isDark ? AppColors.primaryForeground : AppColors.textPrimary;
    ...
    if (isTransfer) {
      ...
      icon = isReceivedTransfer ? Icons.arrow_downward_rounded : Icons.arrow_upward_rounded;
      sign = isReceivedTransfer ? '+' : '-';
    } else if (isDeposit) {
    ...
    if (transaction.description != null) {
      primaryLabel = transaction.description!;
    } else if (isTransfer) {
      final wallets = locator.get<WalletController>().data ?? [];
      if (isReceivedTransfer) {
        final fromWallet = wallets.where((w) => w.id == transaction.walletId).firstOrNull;
        primaryLabel = 'Transfer In';
        secondaryLabel = fromWallet != null ? '← ${fromWallet.name}' : null;
      } else {
        final toWallet = wallets.where((w) => w.id == transaction.toWalletId).firstOrNull;
        primaryLabel = 'Transfer Out';
        secondaryLabel = toWallet != null ? '→ ${toWallet.name}' : null;
      }
    } else {
      primaryLabel = isDeposit ? 'Deposit' : 'Withdraw';
    }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  secondaryLabel ?? DateFormat('h:mm a').format(transaction.date),
                  style: GoogleFonts.dmSans(
                    fontSize: 10,
                    color: secondaryLabel != null ? AppColors.info : AppColors.textTertiary,
                  ),
                ),
              ],
            ),
