...
class _SplitEntry {
  ...
  bool get isValid => amount > 0 && (isPlan || category != null || entityType != null);
...
class _CreateTransactionSheetState extends State<CreateTransactionSheet> {
  ...
  void _toggleSplit() {
    setState(() {
      ...
      if (_splitMode) {
        for (final e in _splitEntries) e.dispose();
        _splitEntries.clear();
        _splitEntries.add(_SplitEntry(type: _type)
          ..amountCtrl.text = _amountCtrl.text);
      } else {
        for (final e in _splitEntries) e.dispose();
        _splitEntries.clear();
      }
  ...
  Future<void> _submit() async {
    if (_selectedWallet == null) { setState(() => _walletError = true); return; }
    if (_amount <= 0) return;
      ...
      try {
        ...
        for (final entry in _splitEntries) {
          ...
          final autoDesc = entry.entityLabel != null
              ? (entry.type == TransactionType.income ? 'From ${entry.entityLabel}' : 'To ${entry.entityLabel}')
              : (entry.type == TransactionType.income ? 'Earns ${entry.category?.name ?? ''}' : 'Pays ${entry.category?.name ?? ''}');
          ...
          final w = allWallets.firstWhere((w) => w.id == wid, orElse: () => WalletModel.fromEntity(_selectedWallet!));
        ...
        for (final kv in walletDeltas.entries) {
          final w = allWallets.firstWhere((w) => w.id == kv.key,
    ...
    if (_entityType == null && _category == null) { setState(() => _categoryError = true); return; }
    ...
    try {
      if (_isPlan) {
        ...
        final autoDesc = _entityLabel != null
            ? (_type == TransactionType.income ? 'From $_entityLabel' : 'To $_entityLabel')
            : (_type == TransactionType.income ? 'Earns ${_category!.name}' : 'Pays ${_category!.name}');
        await _txController.createTransaction(Transaction(
          amount: _amount,
          type: _type,
            ...
            child: ListView(
              ...
              children: [
                ListTile(
                  ...
                  title: Text('No budget', style: GoogleFonts.dmSans(fontSize: 13, color: AppColors.textSecondary)),
                  trailing: _selectedProfileId == null ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent) : null,
                  onTap: () {
                    setState(() { _selectedProfileId = null; _selectedProfileName = null; _selectedPlanMonth = null; _category = null; });
  ...
  Future<void> _applyEntityEffect({
    ...
    final shouldPayDebt = debtId != null && (
      entityType == 'debt_payment' ||
      (entityType == 'lending' && transactionType == TransactionType.income)
    );
  ...
  String _entityDescription(String type) => switch (type) {
    ...
    'lending'       => _type == TransactionType.income
  ...
  String? _buildEntityMeta() {
    ...
    if (_debtId != null) {
      final debt = _debtController.data?.where((d) => d.id == _debtId).firstOrNull;
    ...
    if (_goalId != null) {
      final goal = _goalController.data?.where((g) => g.id == _goalId).firstOrNull;
    ...
    if (_subscriptionId != null) {
      final sub = _subController.data?.where((s) => s.id == _subscriptionId).firstOrNull;
  ...
  void _pickMainEntity() {
    ...
    final types = isIncome
        ? [
            (type: 'lending', icon: Icons.arrow_downward_rounded, label: 'Receivable Collected', color: AppColors.success),
            (type: 'debt_received', icon: Icons.arrow_downward_rounded, label: 'Loan Received', color: AppColors.error),
          ]
        : [
            (type: 'debt_payment', icon: Icons.arrow_upward_rounded, label: 'Debt Payment', color: AppColors.error),
            (type: 'lending', icon: Icons.arrow_downward_rounded, label: 'New Receivable', color: AppColors.success),
            (type: 'goal', icon: Icons.flag_outlined, label: 'Goal', color: AppColors.accent),
            (type: 'subscription', icon: Icons.autorenew_rounded, label: 'Subscription', color: AppColors.info),
          ];
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
        ...
        child: SafeArea(
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: Text('Link To', style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w700, color: textPrimary)),
            ),
              ...
              child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ListTile(
                    ...
                    leading: Icon(Icons.link_off_rounded, color: AppColors.textTertiary, size: 18),
                    title: Text('None', style: GoogleFonts.dmSans(fontSize: 13, color: textPrimary)),
                    trailing: _entityType == null ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent) : null,
                    onTap: () {
                      setState(() { _entityType = null; _entityLabel = null; _debtId = null; _goalId = null; _subscriptionId = null; });
                      Navigator.pop(context);
                    },
                  ),
                  ...types.map((t) => ListTile(
                    ...
                    subtitle: Text(_entityDescription(t.type), style: GoogleFonts.dmSans(fontSize: 11, color: AppColors.textSecondary)),
                    trailing: _entityType == t.type ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent) : null,
                    onTap: () {
                      setState(() { _entityType = t.type; _entityLabel = null; _debtId = null; _goalId = null; _subscriptionId = null; _category = null; });
                      Navigator.pop(context);
                      if (_isMainCreateOnly(t.type)) {
                        _addMainEntityInline(t.type, (id, label) => setState(() {
                          _entityLabel = label;
                          _debtId = id;
                        }));
                      } else {
  ...
  void _pickMainEntityItem(String type) {
    ...
    if (type == 'debt_payment' || type == 'debt_received') {
      final debtType = type == 'debt_received' ? DebtType.lending : DebtType.borrowing;
      items = (_debtController.data ?? [])
          .where((d) => d.type == debtType && d.status != DebtStatus.settled)
          .map((d) => (id: d.id!, label: d.personName, amount: type == 'debt_received' ? 'Expecting ${currencyFormatter.format(d.remainingAmount)}' : 'Still owe ${currencyFormatter.format(d.remainingAmount)}'))
          .toList();
    } else if (type == 'lending') {
      items = (_debtController.data ?? [])
          .where((d) => d.type == DebtType.lending && d.status != DebtStatus.settled)
          .map((d) => (id: d.id!, label: d.personName, amount: 'Expecting ${currencyFormatter.format(d.remainingAmount)}'))
          .toList();
    } else if (type == 'goal') {
      items = (_goalController.data ?? [])
          .where((g) => !g.isCompleted)
          .map((g) => (id: g.id!, label: g.name, amount: '${currencyFormatter.format(g.currentAmount)} of ${currencyFormatter.format(g.targetAmount)} saved'))
          .toList();
    } else {
      items = (_subController.data ?? [])
          .map((s) => (id: s.id!, label: s.name, amount: '${currencyFormatter.format(s.amount)} recurring'))
          .toList();
    }

    final canAdd = type == 'lending' || type == 'debt_received' || type == 'goal' || type == 'subscription';
    ...
    void selectItem(String id, String label) {
      setState(() {
        _entityLabel = label;
        if (type == 'debt_payment' || type == 'debt_received' || type == 'lending') _debtId = id;
        ...
        child: SafeArea(
          ...
          child: Column(children: [
            ...
            Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              child: Row(children: [
                Expanded(child: Text('Select', style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w700, color: textPrimary))),
                if (canAdd)
                  GestureDetector(
                    onTap: () { Navigator.pop(sheetCtx); _addMainEntityInline(type, selectItem); },
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      const Icon(Icons.add_rounded, size: 15, color: AppColors.accent),
                      const SizedBox(width: 3),
                      Text(addLabel, style: GoogleFonts.dmSans(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.accent)),
                    ]),
                  ),
              ]),
            ),
            ...
            Flexible(
              child: items.isEmpty
                  ? Padding(
                      ...
                      child: Text(canAdd ? 'None yet — tap + to create one' : 'No active records to link',
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          contentPadding: EdgeInsets.zero,
                          title: Text(item.label, style: GoogleFonts.dmSans(fontSize: 13, color: textPrimary)),
                          subtitle: Text(item.amount, style: GoogleFonts.dmMono(fontSize: 12, color: AppColors.textSecondary)),
                          trailing: (_debtId == item.id || _goalId == item.id || _subscriptionId == item.id)
                              ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent)
                              : null,
                          onTap: () { selectItem(item.id, item.label); Navigator.pop(sheetCtx); },
                        );
                      },
                    ),
            ),
  ...
  void _addMainEntityInline(String type, void Function(String id, String label) onCreated) {
      ...
      showModalBottomSheet(
        ...
        builder: (_) => AddDebtSheet(
          ...
          onSave: (debt) async {
            ...
            final debtType = type == 'lending' ? DebtType.lending : DebtType.borrowing;
            final created = _debtController.data?.where((d) => d.type == debtType).lastOrNull;
            if (created?.id != null) onCreated(created!.id!, created.personName);
          },
        ),
      );
    } else if (type == 'goal') {
      GoalsManagementDialog.show(context, onSave: (goal) async {
  ...
  Widget build(BuildContext context) {
    ...
    final addLabel = _splitMode
        ? (_splitBalanced && amountLabel != null ? 'Split  $amountLabel' : 'Split Transaction')
        : _isPlan
    ...
    return Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        Padding(
          ...
          child: Row(children: [
            _TypePill(label: 'Expense', color: AppColors.error, selected: _type == TransactionType.expense, onTap: () => setState(() { _type = TransactionType.expense; _category = null; _categoryError = false; _entityType = null; _entityLabel = null; _debtId = null; _goalId = null; _subscriptionId = null; })),
            const SizedBox(width: 8),
            _TypePill(label: 'Income', color: AppColors.success, selected: _type == TransactionType.income, onTap: () => setState(() { _type = TransactionType.income; _category = null; _categoryError = false; _entityType = null; _entityLabel = null; _debtId = null; _goalId = null; _subscriptionId = null; })),
            const Spacer(),
            _PlanTogglePill(
              active: _isPlan,
              onTap: () => setState(() {
            ...
            if (!_isPlan) ...[
              ...
              _Chip(
                ...
                label: 'Split',
                color: _splitMode ? AppColors.accent : AppColors.textSecondary,
                isDark: isDark,
        ...
        Padding(
          ...
          child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
            Text(currencyFormatter.currencySymbol,
                style: GoogleFonts.dmMono(fontSize: 22, fontWeight: FontWeight.w500, color: AppColors.textSecondary)),
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  ...
                  focusedBorder: UnderlineInputBorder(
                    borderSide: BorderSide(color: _typeColor.withValues(alpha: 0.4), width: 1.5),
                  ),
                  isDense: true,
        ...
        if (!_splitMode) SingleChildScrollView(
          ...
          child: Row(children: [
            _Chip(
              ...
              color: _selectedProfileId != null ? AppColors.accent : AppColors.textSecondary,
              isDark: isDark,
              highlighted: false,
            ...
            if (!_splitMode && _entityType == null) ...[
              ...
              _Chip(
                ...
                color: _categoryError ? AppColors.error : (_category != null ? _typeColor : AppColors.textSecondary),
                isDark: isDark,
            ...
            if (!_isPlan && !_splitMode) ...[
              ...
              const SizedBox(width: 8),
              _Chip(
                icon: _entityType != null ? Icons.link_rounded : Icons.link_outlined,
                label: _entityLabel ?? 'Link',
                color: _entityType != null ? AppColors.success : AppColors.textSecondary,
                isDark: isDark,
            ...
            return Padding(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: _typeColor.withValues(alpha: 0.2), width: 0.5),
                ),
                child: Row(children: [
                  Icon(Icons.info_outline_rounded, size: 13, color: _typeColor),
                  const SizedBox(width: 7),
                  Expanded(child: Text(meta, style: GoogleFonts.dmSans(fontSize: 12, fontWeight: FontWeight.w500, color: _typeColor))),
                ]),
              ),
            );
          }),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                GestureDetector(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(Icons.add_circle_outline_rounded, size: 15, color: AppColors.accent),
                    const SizedBox(width: 5),
                    Text('Add line', style: GoogleFonts.dmSans(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.accent)),
                  ]),
                ),
                const Spacer(),
                Text(
                  ...
                  style: GoogleFonts.dmMono(
                    ...
                    color: _splitBalanced ? AppColors.success : (_splitTotal > _amount ? AppColors.error : AppColors.textSecondary),
                  ),
                ),
              ]),
              ...
              ClipRRect(
                ...
                child: LinearProgressIndicator(
                  value: _amount > 0 ? (_splitTotal / _amount).clamp(0.0, 1.0) : 0,
                  minHeight: 4,
                  backgroundColor: borderColor,
                  valueColor: AlwaysStoppedAnimation(
                    _splitBalanced ? AppColors.success : (_splitTotal > _amount ? AppColors.error : AppColors.accent),
                  ),
                ),
              ),
            ]),
            ...
            child: ElevatedButton(
              ...
              style: ElevatedButton.styleFrom(
                backgroundColor: _splitMode
                    ? (_splitBalanced && _selectedWallet != null ? AppColors.accent : AppColors.textTertiary)
                    : (_amount > 0 && (_category != null || _entityType != null || _isPlan) && (_isPlan || _selectedWallet != null)
...
class _SplitEntryRowState extends State<_SplitEntryRow> {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text('Wallet', style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w700, color: textPrimary)),
          const SizedBox(height: 8),
          ...wallets.map((w) => ListTile(
            contentPadding: EdgeInsets.zero,
            leading: Icon(Icons.account_balance_wallet_outlined, color: AppColors.accent, size: 18),
            title: Text(w.name, style: GoogleFonts.dmSans(fontSize: 13, color: textPrimary)),
            subtitle: Text(currencyFormatter.format(w.balance), style: GoogleFonts.dmSans(fontSize: 11, color: AppColors.textSecondary)),
            trailing: widget.entry.walletId == w.id ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent) : null,
            onTap: () { setState(() { widget.entry.walletId = w.id; widget.entry.walletName = w.name; }); Navigator.pop(context); },
          )),
        ]),
      ),
    );
  ...
  void _pickProfile() {
    ...
    for (final p in profiles) {
      if (!p.isActive) continue;
      if (p.isMonthly) {
        final profilePlans = plans.where((pl) => pl.budgetProfileId == p.id && !pl.isClosed && pl.month != null).toList();
        if (profilePlans.isEmpty) {
          items.add((label: p.name, profileId: p.id!));
        } else {
          for (final pl in profilePlans) {
            final dt = DateTime.tryParse('${pl.month!}-01');
            final label = dt != null ? '${p.name} – ${DateFormat('MMM yyyy').format(dt)}' : p.name;
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          Center(child: Container(width: 36, height: 4, decoration: BoxDecoration(color: AppColors.textTertiary.withValues(alpha: 0.4), borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 16),
          Text('Budget', style: GoogleFonts.dmSans(fontSize: 15, fontWeight: FontWeight.w700, color: textPrimary)),
          const SizedBox(height: 8),
          ...items.map((item) => ListTile(
            contentPadding: EdgeInsets.zero,
            leading: Icon(Icons.account_balance_wallet_outlined, color: AppColors.accent, size: 18),
            title: Text(item.label, style: GoogleFonts.dmSans(fontSize: 13, color: textPrimary)),
            trailing: widget.entry.profileId == item.profileId ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent) : null,
            onTap: () { setState(() { widget.entry.profileId = item.profileId; widget.entry.profileName = item.label; }); Navigator.pop(context); },
          )),
        ]),
      ),
    );
              ...
              child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ListTile(
                    ...
                    leading: Icon(Icons.link_off_rounded, color: AppColors.textTertiary, size: 18),
                    title: Text('None', style: GoogleFonts.dmSans(fontSize: 13, color: textPrimary)),
                    trailing: widget.entry.entityType == null ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent) : null,
                    onTap: () {
                      setState(() { widget.entry.entityType = null; widget.entry.entityLabel = null; widget.entry.debtId = null; widget.entry.goalId = null; widget.entry.subscriptionId = null; });
                      Navigator.pop(context);
                    },
                  ),
                  ...types.map((t) => ListTile(
                    ...
                    title: Text(t.label, style: GoogleFonts.dmSans(fontSize: 13, fontWeight: FontWeight.w600, color: textPrimary)),
                    subtitle: Text(_entityDescription(t.type), style: GoogleFonts.dmSans(fontSize: 11, color: AppColors.textSecondary)),
                    trailing: widget.entry.entityType == t.type ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent) : null,
                    onTap: () {
                      setState(() {
                      ...
                      if (_isCreateOnly(t.type)) {
                        _addEntityInline(t.type, (id, label) => setState(() {
  ...
  void _pickEntityItem(String type) {
    ...
    void selectItem(String id, String label) {
      setState(() {
        widget.entry.entityLabel = label;
        if (type == 'debt_payment' || type == 'debt_received' || type == 'lending') widget.entry.debtId = id;
        if (type == 'goal') widget.entry.goalId = id;
        if (type == 'subscription') {
          widget.entry.subscriptionId = id;
          final sub = _subController.data?.where((s) => s.id == id).firstOrNull;
          if (sub != null) widget.entry.amountCtrl.text = sub.amount.toStringAsFixed(2);
          widget.onChanged();
        }
      });
    }
          ...
          child: Column(children: [
            ...
            Padding(
              ...
              child: Row(children: [
                ...
                if (canAdd)
                  GestureDetector(
                    onTap: () { Navigator.pop(sheetCtx); _addEntityInline(type, selectItem); },
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      const Icon(Icons.add_rounded, size: 15, color: AppColors.accent),
                      const SizedBox(width: 3),
                      Text(addLabel, style: GoogleFonts.dmSans(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.accent)),
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return ListTile(
                          ...
                          title: Text(item.label, style: GoogleFonts.dmSans(fontSize: 13, color: textPrimary)),
                          subtitle: Text(item.amount, style: GoogleFonts.dmMono(fontSize: 12, color: AppColors.textSecondary)),
                          trailing: (widget.entry.debtId == item.id || widget.entry.goalId == item.id || widget.entry.subscriptionId == item.id)
                              ? const Icon(Icons.check_rounded, size: 16, color: AppColors.accent)
                              : null,
                          onTap: () { selectItem(item.id, item.label); Navigator.pop(sheetCtx); },
                        );
  ...
  void _addEntityInline(String type, void Function(String id, String label) onCreated) {
    if (type == 'lending' || type == 'debt_received') {
      ...
      showModalBottomSheet(
        ...
        builder: (_) => AddDebtSheet(
          ...
          onSave: (debt) async {
            await _debtController.createDebt(debt);
            final debtType = isReceivable ? DebtType.lending : DebtType.borrowing;
            final created = _debtController.data?.where((d) => d.type == debtType).lastOrNull;
            if (created?.id != null) onCreated(created!.id!, created.personName);
          },
        ),
  ...
  Widget build(BuildContext context) {
    ...
    final typeColor = widget.entry.type == TransactionType.income ? AppColors.success : AppColors.error;
    final hasCategory = widget.entry.category != null;
    final now = DateTime.now();
    final isToday = DateUtils.isSameDay(widget.entry.date, now);
    final dateLabel = isToday ? 'Today' : DateFormat('MMM d').format(widget.entry.date);

    return Container(
      margin: const EdgeInsets.only(bottom: 8),
      padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8),
      decoration: BoxDecoration(
        color: widget.isDark ? Colors.white.withValues(alpha: 0.04) : AppColors.backgroundSecondary,
        borderRadius: BorderRadius.circular(12),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(
                  widget.entry.type == TransactionType.income ? Icons.arrow_downward_rounded : Icons.arrow_upward_rounded,
                  size: 11, color: typeColor,
                ),
                const SizedBox(width: 3),
                Text(
                  widget.entry.type == TransactionType.income ? 'In' : 'Out',
          ...
          Expanded(
            child: TextField(
              ...
              decoration: InputDecoration(
                ...
                border: InputBorder.none,
                hintText: hasCategory ? widget.entry.category!.name : 'Description',
                hintStyle: GoogleFonts.dmSans(
                  fontSize: 13,
                  color: hasCategory ? textPrimary.withValues(alpha: 0.5) : AppColors.textTertiary,
                ),
              ),
            ),
          ),
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              style: GoogleFonts.dmMono(fontSize: 13, fontWeight: FontWeight.w600, color: textPrimary),
              decoration: InputDecoration(
                isDense: true,
                hintText: '0.00',
                hintStyle: GoogleFonts.dmMono(fontSize: 13, color: AppColors.textTertiary),
                border: InputBorder.none,
                contentPadding: EdgeInsets.zero,
              ),
            ),
        ...
        Wrap(spacing: 5, runSpacing: 5, children: [
          _SplitChip(
            ...
            label: 'Plan',
            color: widget.entry.isPlan ? AppColors.accent : AppColors.textSecondary,
          ...
          _SplitChip(
            ...
            color: widget.entry.profileName != null ? AppColors.accent : AppColors.textSecondary,
            isDark: widget.isDark,
            onTap: _pickProfile,
          ),
          if (!widget.entry.isPlan && widget.entry.entityType == null)
            _SplitChip(
              ...
              color: widget.entry.category != null ? AppColors.textSecondary : AppColors.textTertiary,
              isDark: widget.isDark,
          ...
          _SplitChip(
            ...
            color: widget.entry.walletName != null ? AppColors.accent : AppColors.textSecondary,
            isDark: widget.isDark,
            onTap: _pickWallet,
          ),
          _SplitChip(
            ...
            onTap: () async {
              ...
              if (picked != null) setState(() => widget.entry.date = picked);
            },
          ),
          if (!widget.entry.isPlan)
            _SplitChip(
              icon: widget.entry.entityType != null ? Icons.link_rounded : Icons.link_off_rounded,
              label: widget.entry.entityLabel ?? (widget.entry.entityType != null ? _entityTypeLabel(widget.entry.entityType!) : 'Link'),
              color: widget.entry.entityLabel != null ? AppColors.success : AppColors.textSecondary,
              isDark: widget.isDark,
              onTap: _pickEntity,
            ),
        ]),
        ...
        Builder(builder: (_) {
          ...
          return Padding(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: typeColor.withValues(alpha: widget.isDark ? 0.1 : 0.06),
                borderRadius: BorderRadius.circular(8),
                border: Border.all(color: typeColor.withValues(alpha: 0.2), width: 0.5),
              ),
              child: Row(children: [
                Icon(Icons.info_outline_rounded, size: 12, color: typeColor),
                const SizedBox(width: 6),
                Expanded(child: Text(meta, style: GoogleFonts.dmSans(fontSize: 11, fontWeight: FontWeight.w500, color: typeColor))),
              ]),
            ),
          );
        }),
  ...
  String _entityDescription(String type) => switch (type) {
    'debt_payment'  => 'Pay back an existing debt',
    'lending'       => widget.entry.type == TransactionType.income
        ? 'Select which receivable was collected'
        : 'Record money you lent — creates a new receivable',
    'debt_received' => 'Record a loan you received — creates a new debt',
    'goal'          => 'Contribute toward a savings goal',
  ...
  String? _buildMeta() {
    ...
    if (e.debtId != null) {
      final debt = _debtController.data?.where((d) => d.id == e.debtId).firstOrNull;
      if (debt == null) return null;
    ...
    if (e.goalId != null) {
      final goal = _goalController.data?.where((g) => g.id == e.goalId).firstOrNull;
      if (goal == null) return null;
      final remaining = (goal.targetAmount - goal.currentAmount).clamp(0.0, double.infinity);
      return '${currencyFormatter.format(goal.currentAmount)} of ${currencyFormatter.format(goal.targetAmount)} saved · ${currencyFormatter.format(remaining)} to go';
    }
    if (e.subscriptionId != null) {
      final sub = _subController.data?.where((s) => s.id == e.subscriptionId).firstOrNull;
      if (sub == null) return null;
      return '${currencyFormatter.format(sub.amount)} recurring';
    }
    return null;
...
class _SplitChip extends StatelessWidget {
  ...
  const _SplitChip({required this.icon, required this.label, required this.color, required this.isDark, this.onTap});
